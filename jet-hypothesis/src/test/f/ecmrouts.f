c
c Copyright (c) James G. MacKinnon, 1999 (corrected 2001-1-8)
c
c ecmrouts.f: This is a set of subroutines to estimate critical values
c and P values for ECM  cointegration tests. It is written in
c Fortran 77. Simply call ecmval, specifying the first seven arguments.
c The result comes back in the last argument.
c
c A standalone program called ecmdist.f is also available.c
c Those who wish to incorporate these routines into their own programs
c should use the routines in ecmrouts.f instead.
c
...
c
c Neil R. Ericsson and James G. MacKinnon (1999) "Distributions of Error
c Correction Tests for Cointegration", International Finance Discussion Paper
c No. 655, Board of Governors of the Federal Reserve System, Washington,
c D.C., December.
c
...
c
c The program must have access to the files probs.tab and ecm-#.tab
c for # = 01, 02, 03 ..., 12. As currently written, these files must be
c in the current directory or in the directory /usr/local/urcdist.
c Make sure that these files are in the proper format for your computer (i.e.
...
c and by LF alone for Unix systems).
      ...
      call ecmval(niv,itv,nc,nobs,ncoef,isave,arg,cval)
         write(6,88) arg, cval
   88     format(' At level ', f4.2, ' finite-sample critical value = ',
     &      f15.8)
     
      stop
	  end
      subroutine ecmval(niv,itv,nc,nobs,ncoef,isave,arg,val)
c
c niv = # of integrated variables, (1 to 12)
c itv = 1, 2, 3, 4 for nc, c, ct, ctt
c nc = 1 or 2 for critical value or P value
c nobs = sample size (0 for asymptotic)
c ncoef = total number of coefficients (if nobs > 0), or 0.
c isave = 0 or 1. If isave=1, this call to urcval has the same values of
c                 niv and itv as the last one, so data files do not
...
c
      ...
      fnames(8) = 'ecm-08.tab'
      fnames(9) = 'ecm-09.tab'
      fnames(10) = 'ecm-10.tab'
      fnames(11) = 'ecm-11.tab'
      fnames(12) = 'ecm-12.tab'
c
...
c
  ...
  302 format(8x,3i3,i5)
      if (model.eq.2.or.model.eq.4.or.model.eq.6) then
        nvar = 3
      else
        nvar = 4
      end if
c
c Modify nobs if number of estimated coefficients is not the usual one.
c
      if (nobs.gt.0.and.ncoef.gt.0) nobs = nobs - ncoef + nz + nreg + 1
c
      do i=1,221
...
c
        do i=1,np
          ic = imin - nph - 1 + i
          call eval(beta(1,ic),crits(ic),model,nreg,nz,nobs)
          yvect(i) = crits(ic)
          xmat(i,1) = 1.d0
          xmat(i,2) = cnorm(ic)
...
c
        ...
        if (imin.lt.np) then
          ...
          if (np1.lt.5) np1 = 5
          do i=1,np1
            call eval(beta(1,i),crits(i),model,nreg,nz,nobs)
            yvect(i) = crits(i)
            xmat(i,1) = 1.d0
            xmat(i,2) = cnorm(i)
          ...
          if (np1.lt.5) np1 = 5
          do i=1,np1
            call eval(beta(1,222-i),crits(222-i),model,nreg,nz,nobs)
            ic = 222 - i
            yvect(i) = crits(ic)
            xmat(i,1) = 1.d0
            xmat(i,2) = cnorm(ic)
...
c
      ...
      subroutine eval(beta,cval,model,nreg,nz,nobs)
      implicit real*8 (a-h,o-z)
c
c Copyright (c) James G. MacKinnon, 1997
c Routine to evaluate response surface for specified betas and sample size.
c Version for ecm tests.
c
      real*8 beta(4)
      if (nobs.eq.0) then
        cval = beta(1)
